...
- Background copy is capability-gated: a detached spawn is reported as session-only and cannot promise unattended overnight completion. Tests: installer **64/64** node tests, including unrelated 200/404 listeners, timeouts/throws, prior-vs-new resource rollback, cleanup failures, and copy branches. Live `systemd-analyze verify` remains silent for the shipped unit.
