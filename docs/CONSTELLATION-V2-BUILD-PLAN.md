...
- Windows startup is transactional: the installer snapshots any existing task, launcher/XML, and firewall rule; `/Create`, `/Run`, firewall, or product-readiness failure restores the prior resources or removes only resources created by the failed attempt. Cleanup continues after individual restore/delete failures and reports every failure.
- Wizard startup claims require the exact Constellation `/wall` readiness contract (200 + HTML + Constellation title), not merely an HTTP response or a successful process/task launch. Linux now reports a verified start, and login-only startup is distinct from boot startup when `loginctl enable-linger` fails.
- Background copy is capability-gated: a detached spawn is reported as session-only and cannot promise unattended overnight completion. Tests: installer **71/71** node tests, including one-deadline `/wall` response handling, aborted/oversized streams, exact Linux state restoration with aggregated failures, atomic Windows file-write rollback, unrelated 200/302/404 listeners, timeouts/throws, prior-vs-new resource rollback, cleanup failures, and copy branches. Live `systemd-analyze verify` remains silent for the shipped unit.
