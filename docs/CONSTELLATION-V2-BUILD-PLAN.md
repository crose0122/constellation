...
### Q5 status — legal draft confirmed implementation-ready, awaiting founder sign-off (2026-09-25)
- Lex's draft (#AMS #1300, 2026-09-23) is the candidate text for spec §7.5. Lex has **confirmed the T5 implementation constraint as asked in #AMS #1511** (his reply #AMS #1519, 2026-09-25). What the CP8 gate may now cite as agreed:
  1. **Compile-time constants.** T_HIGH/T_MED/T_ADULT live in the policy module source, versioned **with** the classifier model (same release artifact). A threshold change without a model-version bump is itself a test failure. Tuning only via founder sign-off recorded in the spec changelog — no runtime config knob, no API.
  2. **No runtime mutation path.** Read-only constants: no setter, no config loader, no env override, no IPC/RPC writer. The release process is the only writer of the policy file. T5's grep-level test asserts the negative: no assignment-to-threshold outside the declaration block (e.g. `T_HIGH\s*=[^=]` outside declarations fails CI), plus an integration assertion that a running instance cannot change effective thresholds via any input channel.
  3. **Escalation record as proof.** Every escalation record logs model ID + version, class scores, and the threshold values that fired; a record that disagrees with its model version's compiled constants fails T5.
  4. **No-disable interlock.** The same grep test also asserts the absence of any disable/bypass flag on the escalation path (tuning adjusts thresholds; it never turns the rule off).
- **Fold-in drafting note (Lex):** when sections 1, 2, 5 and the T1–T6 hooks are folded into §7.5 post-sign-off, keep **verbatim**: (a) constraint 1's founder-changelog requirement, and (b) the section-5 boundary statement (door-2 and existing-library scope exclusions) — those two sentences are the basis of the family-privacy posture. Lex confirms T2 and T6 are already written as absolute assertions in his T-hooks.
- **Still blocking CP8 auto-routing:** founder sign-off recorded in the spec changelog (founder action). Until then: no fold-in of the policy text into the spec, auto-routing stays off, folder declaration only — per #AMS #1511 and this plan's CP8 line above. Lex re-issues if the founder amends the draft; the T5 confirmation stands regardless.
- **Lex verification of this entry (2026-09-25, AMS #1523 → lex):** the four numbered constraints and the fold-in note above match #AMS #1519 (and draft #1300's §1 constraints and §5 boundary) with no substantive drift. Two caveats reported back on the bus: (1) this commit was `[ahead 1]` of `origin/feat/v2-unify` at verification time — until pushed, the entry is citable locally but not on GitHub; (2) #1523's "byte-identical on feat/v2-branding-repair" note is now stale — the Q5 block exists on feat/v2-unify only, so cherry-pick to branding-repair only if that branch's CI needs it.
